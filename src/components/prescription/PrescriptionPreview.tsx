...
export function PrescriptionPreview({ rx, patient }: { rx: Prescription; patient: Patient }) {
  ...
  return (
    <div className="print-area bg-white text-foreground rounded-2xl border shadow-lg overflow-hidden mx-auto max-w-2xl" style={{ aspectRatio: "8.5 / 11" }}>
      {/* Header band */}
      <div className="px-8 py-6 flex items-center justify-between" style={{ background: `linear-gradient(135deg, oklch(${branding.primary}), oklch(${branding.accent}))`, color: "white" }}>
      ...
      <div className="px-8 py-3 text-center text-xs italic" style={{ background: `oklch(${branding.surface})`, color: `oklch(${branding.accent})` }}>
        {branding.rxFooter}
      </div>
    </div>
  );
