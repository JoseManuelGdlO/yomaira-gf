...
export const Route = createFileRoute("/agendar/$slug")({
  head: ({ params }) => ({ meta: [{ title: `Agendar cita — ${params.slug}` }] }),
  component: AgendarSlugPage,
});

type Step = "phone" | "patient" | "slot" | "done";

function applyPublicBranding(b: PublicBrandingDTO) {
  if (typeof document === "undefined" || !b.primary) return;
  const r = document.documentElement.style;
  r.setProperty("--primary", `oklch(${b.primary})`);
  r.setProperty("--ring", `oklch(${b.primary})`);
  if (b.secondary) r.setProperty("--secondary", `oklch(${b.secondary})`);
  if (b.accent) r.setProperty("--accent", `oklch(${b.accent})`);
  if (b.surface) r.setProperty("--surface", `oklch(${b.surface})`);
}
...
function AgendarSlugPage() {
  ...
  useEffect(() => {
    api.publicBooking
      ...
      .then((b) => {
        setBranding(b);
        applyPublicBranding(b);
      })
      .catch(() => setBranding({ clinicName: "Consultorio", slug }));
  }, [slug]);
  ...
  const lookup = async () => {
    ...
    setBusy(true);
    try {
      const found = await api.publicBooking.lookupPatient(slug, phone);
      if (found) {
        setPatient(found);
        setIsNew(false);
      } else {
  ...
  const book = async () => {
    ...
    try {
      const body = patient
        ? { slug, patientId: patient.id, date, time, reason }
        : { slug, name, guardian, phone, date, time, reason };
